...
ISR(TIMER1_COMPA_vect)
{
  ...
  if (st.counter_x > st.exec_block->step_event_count) {
    ...
    // added by bg4dzy for X AXIS
    // 不更新系统位置数据,直至所有的回程补偿步进脉冲执行完成
    if (!st.exec_block->back_lash_comp) {
		//printString("this is normal Steps,isn't BC Steps,should update system position");
