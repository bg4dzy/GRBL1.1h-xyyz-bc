...
void backlash_comp(float *target, plan_line_data_t *pl_data)
{
	...
	for (idx = 0;idx<N_AXIS;idx++) {
		...
		step_diff = step_diff > 0 ? 1 : 
						( step_diff < 0 ? -1 : 0 );
		//if last direction is different than the new direction, and its not 0, we need to comp.
		if ( back_lash_compensation.last_comp_direction[idx] != 0 
		  &&   step_diff != 0
		  && ( back_lash_compensation.last_comp_direction[idx] != step_diff)) {
			// print： axis 0 set for comp
			// printString("axis "); printInteger(idx); printString(" set for compensat");			
			back_lash_compensation.comp_per_axis_mm[idx] = settings.backlash[idx] * step_diff;
			needs_comp = 1;
		}
	}
	if (needs_comp) {
